...
const getListEtudiant = (data, result) => {
    sql.query(`select u.cin,u.nom,u.prenom,p.time from etudiant et ,presence_etudiant p,enseignement en,user u 
    where 
     en.id_matiere = ${data.body.id_matiere}
     and en.id_classe = ${data.body.id_classe}
     and en.id_enseignement = p.id_enseignement
     and et.id_departement = ${data.body.id_departement}
     and et.id_etudiant = p.id_etudiant 
     and p.id_etat_presence_etd = 2
     and p.time >= ${data.body.date}
     and u.id_user = et.id_user	`, (err, res) => {
