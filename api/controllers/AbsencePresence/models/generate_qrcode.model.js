...
const insertPresenceEnseignant = (resultQrCode,callback)=>{
    if(resultQrCode==="Data Enseignement Not Found"){
        callback(null,"Data Enseignement Not Found");
        return;                                                            
    }
    let dateOfQrCode =extractDateFromQrCode(resultQrCode[0].qr_code);
    if(getCurrentDate()!=dateOfQrCode){
        console.log("you can insert now");
        sql.query(`insert presence_enseignant (id_enseignement,etat) values(${resultQrCode[0].id_enseignement},1)`,(err,res)=>{
            if (err) {
                console.log("error:", err);
                callback(err,null);
                return;
            } else {
                callback(null,"sucess insert");
            }
        });
    }else {
        console.log("you can not insert now");
        callback(null,"you can not insert now");
    }
  
}

const updateEnseignement = (resultQrCode,id_enseignant,id_seance, callback) => {
    if(resultQrCode==="Data Enseignement Not Found"){
        callback(null,"Data Enseignement Not Found");
        return;
    }
...
const extractDateFromQrCode=(qr_code)=>{
    let resultDate = qr_code.split('-');
    return `${resultDate[8]}-${resultDate[9]}-${resultDate[10]}`
}
